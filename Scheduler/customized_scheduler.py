...
def Scheduler(optimizer, lr, epochs):
    # Parameters: optimzer, init_decay_epochs, min_decay_lr, restart_interval, restart_interval_multiplier,
    # restart_lr, warmup_epochs, warmup_start_lr, last_epoch, verbose
    sche_fn = CyclicCosineDecayLR(optimizer=optimizer,
                                init_decay_epochs=int(epochs/2),
                                min_decay_lr=1e-6,
                                restart_interval=int(epochs/10),
                                restart_interval_multiplier=2.0,
                                restart_lr=lr/4,
                                warmup_epochs=int(epochs/25),
                                warmup_start_lr=1e-6,
                                last_epoch=-1,
                                verbose=False)
    
    print('Initialised customized scheduler')
    print('(Warm-up, Initial-decay, and geometrically increased restart interval)')
